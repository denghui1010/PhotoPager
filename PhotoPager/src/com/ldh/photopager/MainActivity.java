...
public class MainActivity extends Activity {
    ...
    private void addView(ViewGroup viewGroup, int len) {
        for (int i = 1; i < len + 1; i++) {
            TextView textView = new TextView(MainActivity.this);
            textView.setText("测试测试测试测试测试测试" + i);
...
//    lv_test.setAdapter(new MyAdapter());
        ScrollLayout photoPager = new ScrollLayout(MainActivity.this);
        addView(photoPager, 3);
        photoPager.setLayoutParams(new AbsListView.LayoutParams(LayoutParams.MATCH_PARENT, 300));
        lv_test.addHeaderView(photoPager);
        lv_test.setAdapter(new MyAdapter());
    }
